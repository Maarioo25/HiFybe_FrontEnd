  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError(err.response?.data?.mensaje || t('friends.errorLoading'));
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-harmony-accent/40 scrollbar-track-transparent">
        <div className="container mx-auto px-6">
          <div className="bg-harmony-secondary/30 backdrop-blur-sm rounded-2xl border border-harmony-text-secondary/10 mt-6 mb-6">
            <div className="p-6">
              <h2 className="text-xl font-bold text-harmony-accent mb-6">
                {t('friends.title')}
              </h2>
              ...
              <div className="h-[46vh] max-h-[65vh] overflow-y-auto scrollbar-thin scrollbar-thumb-harmony-accent/40 scrollbar-track-transparent px-4 mb-8">
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {(friendsList.length === 0 ? [{ id: 'add' }] : [...friendsList, { id: 'add' }]).map(
                      (amigo) =>
                        amigo.id === 'add' ? (
                          <div
                            key="add"
                            className="friend-card relative group w-full h-50"
                  ...
                  <div className="space-y-4">
                    {solicitudes.length === 0 ? (
                      ...
                      solicitudes.map((s) => (
                        <div
                          key={s._id}
                          className="bg-harmony-secondary/20 p-4 rounded-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow border border-harmony-secondary/30"
                        >
                          <div className="flex items-center gap-4">
                            <img
                              src={s.de_usuario_id?.foto_perfil || ''}
                              alt={s.de_usuario_id?.nombre || t('friends.unknownUser')}
                            ...
                            <span className="text-white text-lg font-semibold truncate max-w-[180px]">
                              {s.de_usuario_id?.nombre || t('friends.unknownUser')}
                            </span>
                          </div>
                          <div className="flex gap-3">
                            ...
                            <button
                              onClick={() => handleRespondRequest(s._ld, 'rechazada')}
